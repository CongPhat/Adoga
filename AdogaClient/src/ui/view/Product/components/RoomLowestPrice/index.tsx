...
const RoomLowestPrice = ({ productId }: IRoomLowestPrice) => {
  ...
  const dataRender: RoomEntities = checkEffectData(status == "loading", value)
    ? new RoomEntities({
      ...
      <div className="border border-red-1100 border-t-0 overflow-hidden rounded-md rounded-t-none">
        ...
        >
          {loading ? (
            ...
            <img
              src={dataRender.images[0].linkImage}
              alt={dataRender.images[0].title}
        ...
        <div className="p-4">
          {loading ? (
            ...
            <>
              <h6 className="text-black">{dataRender.name}</h6>
              <div className="text-xs">
                <FontAwesomeIcon icon={faUsers} className="mr-1" />
                {dataRender.people} adult
              </div>
              <p
            ...
            <>
              ...
              <div className="flex items-end flex-col">
                <p className=" line-through font-normal text-sm mt-4">
                  {dataRender.discount == 0
                    ? ""
                    : `đ ${parseMoneyVND(dataRender.price)}`}
                </p>
                <p className=" ">
                  đ
                  <span className="mr-1 text-red-1100 font-bold text-base">
                    {dataRender.discount != 0
                      ? `${parseMoneyVND(
                          (dataRender.price * dataRender.discount) / 100
                        )}`
                      : `${parseMoneyVND(dataRender.price)}`}
                  </span>
                </p>
              </div>
            </>
