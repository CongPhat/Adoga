...
const DayPickerComponent = ({ initType = "monthly", register }: Iprops) => {
  ...
  return (
    <>
      ...
      <div
        className={`flex overflow-hidden  w-full ${ type == "weekly" ? "h-auto" : "h-350"
          }`}
      >
        {type == "weekly" ? (
          <>
            ...
            <>
              <DayPicker
                ...
                className={`${ type === "monthly" && "header-invisible"
                  } focus:outline-none ${ type === "yearly" &&
                  `${ controllMonth.next && "DayPicker-button-next-disable" } ${ controllMonth.pre && "DayPicker-button-pre-disable"
                  }`
                  }`}
              />
              <div
